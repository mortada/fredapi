...
class HTTPCall(object):
    ...
    def __init__(self, rel_url, response=None, side_effect=None):
        ...
        """
        self.url = "{}/{}&api_key={}".format(self.root_url, rel_url, fred_api_key)
        self.response = response
        self.side_effect = side_effect


sp500_obs_call = HTTPCall(
    "series/observations?series_id=SP500&{}&{}".format(
        "observation_start=2014-09-02", "observation_end=2014-09-05"
    ),
...
class TestFred(unittest.TestCase):
    ...
    def test_get_series(self, urlopen):
        ...
        serie = self.fred.get_series(
            "SP500", observation_start="9/2/2014", observation_end="9/5/2014"
        )
    ...
    def test_invalid_id_in_get_series(self, urlopen):
        ...
        url = "{}/series/observations?series_id=invalid&api_key={}".format(
            self.root_url, fred_api_key
        )
    ...
    def test_invalid_id_in_get_series_info(self, urlopen):
        ...
        url = "{}/series?series_id=invalid&api_key={}".format(
            self.root_url, fred_api_key
        )
    ...
    def test_search(self, urlopen):
        ...
        pi_series = self.fred.search_by_release(
            175, limit=3, order_by="series_id", sort_order="asc"
        )
        urlopen.assert_called_with(search_call.url)
        actual = str(
            pi_series[["popularity", "observation_start", "seasonal_adjustment_short"]]
        )
